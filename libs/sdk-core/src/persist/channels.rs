        ...
        let channels: Vec<Channel> = stmt
            .query_map([], |row| {
                ...
                Ok(Channel {
                    ...
                    local_balance_msat: row.get(4)?,
                    receivable_msat: row.get(5)?,
                    closed_at: row.get(6)?,
                    funding_outnum: row.get(7)?,
                    alias_local: row.get(8)?,
                    alias_remote: row.get(9)?,
                    closing_txid: row.get(10)?,
                    htlcs: Vec::new(),
                })
            })?
            .map(|i| i.unwrap())
