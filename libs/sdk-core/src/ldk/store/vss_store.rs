...
impl<P: RetryPolicy<E = VssError> + Send + Sync> VersionedStore for VssStore<P> {
    ...
    async fn put(&self, key: String, value: Vec<u8>, version: i64) -> Result<(), Error> {
        ...
        let key_value = KeyValue {
            key: obfuscated_key,
            version,
            // Persist the next expected version along with the encrypted payload to cross-check
...
// Copied from https://github.com/lightningdevkit/ldk-node/blob/main/src/io/vss_store.rs#L577
